...
def pipeline(data, filename):
    print("Step 1:Uppercase Cols\n\n")
    data = uppercase_strings(data)
...
def undersample_with_nearmiss(X, y):
    ...
    if (len(np.unique(Y_resampled, return_counts=True)[1]) != 2 or
            np.unique(Y_resampled, return_counts=True)[1][0] != minority_class_count):
...
def main():
    ...
    pipeline(df, '../../models/fraud_model.sav')
